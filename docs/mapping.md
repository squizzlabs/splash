...
The renderer uses native SVG rather than a graph library. Layout is a live-rooted breadth-first chain: the focused pilot's current system stays at the top, each jump moves one row downward, and adjacent branches spread horizontally below it. Connection life, mass, and size are encoded on the line so degraded links can be read without opening the inspector.
